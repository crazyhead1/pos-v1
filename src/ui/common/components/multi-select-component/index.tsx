...
const MultiSelectComponent: React.FC<ComponentProps> = ({
  ...
  return (
    <div ref={node}>
      ...
      {optional && (
        <span style={{ fontSize: 13, marginLeft: 10 }}>(Optional)</span>
      )}
      <div
        role="button"
      ...
      >
        <div className={classes.searchContainer}>
          <input
            ...
            // }}
            placeholder={
              (selected.length > 0 && `${selectedItems} ${addSelectedItems}`) ||
              placeholder ||
              "Select..."
            }
          />
        ...
        {hover && (
          <div className={classes.selectContainer}>
            {!disableSelectAll && (
              <div
                className={`${classes.buttonContainer} ${classes.checkAllContainer}`}
              >
                <button type="button" onClick={() => selectAll()}>
                  {options.length === selected.length ? (
                    <div className={classes.check}>deselect</div>
                  ) : (
                    <div className={classes.uncheck} />
                  )}
                </button>
                <span>
                  {selectAllTitle === undefined ? "Select All" : selectAllTitle}
                </span>
              </div>
            )}
            {filter.map((option: OptionValue, index: number) => {
              return (
                ...
                >
                  ...
                  {option.rightIcon && (
                    <img src={option.rightIcon} alt="right" />
                  )}
                </div>
              );
            })}
